...
router.put("/farms/:farmId/animals/:animalId", requireAuth, requireFarmAccess, async (req, res) => {
  try {
    ...
    const { photoUrl, customTag, species, breed, name, sex, dateOfBirth, status, motherId, fatherId, currentZoneId, notes } = req.body;

    const updated = await db.update(animalsTable)
      .set({ photoUrl, customTag, species, breed, name, sex, dateOfBirth, status, motherId, fatherId, currentZoneId, notes, updatedAt: new Date() })
      .where(and(eq(animalsTable.id, animalId), eq(animalsTable.farmId, farmId)))
      .returning();
