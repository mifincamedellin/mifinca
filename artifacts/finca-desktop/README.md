...
Electron desktop app for miFinca — offline-capable farm management for Colombian farms.

## Required environment variables

| Variable | When needed | Description |
|---|---|---|
| `MIFINCA_API_URL` | Build / runtime | Base URL of the miFinca API server. Default: `https://mifinca.replit.app`. Set at build time via `electron-builder extraMetadata` or at runtime via env. |
| `MIFINCA_UPDATE_URL` | Production packaging | URL prefix where `latest-mac.yml` / `latest.yml` update manifests are hosted. Injected by the build script from the object-storage public path. |
...
| `MIFINCA_DEV` | Development | Set to `1` to run the app in dev mode (skips auto-updater, uses local web-dist). |
...
|---|---|
| `VITE_DESKTOP_DOWNLOAD_MAC` | `https://storage.googleapis.com/<bucket>/releases/latest/miFinca.dmg` |
| `VITE_DESKTOP_DOWNLOAD_WIN` | `https://storage.googleapis.com/<bucket>/releases/latest/miFinca-Setup.exe` |
...
- **CORS interceptor**: The Electron session injects `Access-Control-Allow-Origin: *`
  ...
  `MIFINCA_API_URL` in a future update.
